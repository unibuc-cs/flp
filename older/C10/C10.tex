...
$$
\forall (\neg Q_1\vee\ldots \vee \neg Q_n) 
\equiv \neg \exists (Q_1\wedge \ldots \wedge  Q_n)
$$
...
Pentru un program logic definit $KB$  avem \intens{Regula de deducție \textit{backchain}:}
...
\begin{itemize}
  \item \intens{O derivare} din $KB$ a întrebării prin rezoluție SLD este o secvență 
\begin{center}
...
\vspace{.2cm}
\item  Există soluție pentru întrebarea \intens{$Q_1\wedge\cdots\wedge Q_m$}.
\end{enumerate}
\end{frame}
...
\begin{itemize}
  \item Am afirmat că \intens{sistemul de inferență din spatele Prolog-ului este complet}. 
  \begin{itemize}
...
{\small%
\begin{tabular}{l@{\extracolsep{10em}}l}
  1. $p(X,Y) \vee \neg q(X,Z)\vee\neg r(Z,Y)$   & 7. $s(X) \vee\neg t(X,a)$ \\
  2. $p(X,X) \vee\neg s(X)$                     & 8. $s(X) \vee\neg t(X,b)$ \\
  3. $ q(X,b)$                                  & 9. $s(X) \vee\neg t(X,X)$ \\
  4. $ q(b,a)$                                  &10. $t(a,b)$ \\
...
\begin{frame}{Rezolvare prin rezoluție}
  \begin{block}{Clauze asociate}
    \begin{enumerate}
      \item $\forall as(pm(X), Y) \vee \neg vc(X) \vee \neg mr(Y, X)$
      \item $\forall vc(X) \vee \neg ce(ct(Y), X) \vee \neg mr(Y, Y)$
...
\only<5>{3}%
\only<6>{4}
 și redenumim variabilele

\only<2>{$\forall \alert{as(pm(X_1), Y_1)} \vee \structure{\neg vc(X_1) \vee \neg mr(Y_1, X_1)}$}%
\only<3>{$\forall \alert{vc(X_2)} \vee \structure{\neg ce(ct(Y_2), X_2) \vee \neg mr(Y_2, Y_2)}$}%
\only<4>{$\forall \alert{ce(ct(ol), rb)}$}%
\only<5>{$\forall \alert{mr(ol, ol)}$}%
\only<6>{$\forall \alert{mr(rb, rb)}$}

unificăm 
\only<2>{$\alert{as(X, Y)}$}%
\only<3>{$\alert{vc(X_1)}$}%
\only<4>{$\alert{ce(ct(Y_2), X_1)}$}%
\only<5>{$\alert{mr(ol, ol)}$}%
\only<6>{$\alert{mr(Y, rb)}$}
 cu %
\only<2>{$\alert{as(pm(X_1), Y_1)}$}%
\only<3>{$\alert{vc(X_2)}$}%
\only<4>{$\alert{ce(ct(ol), rb)}$}%
\only<5>{$\alert{mr(ol, ol)}$}%
\only<6>{$\alert{mr(rb, rb)}$}
 și obținem substituția

\only<2>{$X \mapsto pm(X_1), Y_1 \mapsto Y$}%
\only<3>{$X_2 \mapsto X_1$}%
\only<4>{$X_1 \mapsto rb, Y_2\mapsto ol$}%
\only<5>{identitate}%
\only<6>{$Y \mapsto rb$}
...
$[X \mapsto pm(rb), Y_1\mapsto rb, X_2\mapsto rb, Y_2\mapsto ol]$
